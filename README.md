...
- **Automated scanning:** `@axe-core/playwright` runs WCAG 2 A/AA scans against every major page (`/login`, `/dashboard`, `/puzzles`, `/training/:id`, `/settings`, `/register`, `/verify-email`) as part of the e2e suite, which has already caught and fixed several real issues (color-contrast on dashboard/settings headings and the ECO-code chip, and an `aria-required-attr` violation). Automated scans are no substitute for manual screen-reader/keyboard testing, though. See [react/e2e/README.md](./react/e2e/README.md) for the full picture, including one known, unfixed contrast violation (`.site-header-version`) currently excluded from the scan.
