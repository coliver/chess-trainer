...
The UI chrome (header, auth pages, dashboard, training, and puzzles pages) is localized via [`react-i18next`](https://react.i18next.com/), with translation resources in [`packages/i18n-locales/locales/*.json`](../packages/i18n-locales) — a directory shared outside `react/` so other consumers can read the same source of truth — over 30 locales including `en-US`, `es`, `fr`, `de`, `it`, `nl`, `pl`, `pt`, `pt-BR`, `ru`, `tr`, `zh-CN`, `ja`, `ko`, `hi`, `ar`, `cs`, `da`, `el`, `fi`, `he`, `hu`, `ms`, `no`, `ro`, `sk`, `sv`, `uk`, `vi`, `id`, plus novelty variants like `en-x-pirate`, `kl` (Klingon), and `en-x-groot`. `src/i18n/i18n.ts` initializes `i18next` (imported once in `main.tsx`, and again in `vitest.setup.ts` so component tests render real strings instead of raw keys), auto-discovers every locale file in that shared directory via `import.meta.glob` (no per-language wiring needed), and reads/writes the chosen language to `localStorage` under the `language` key, mirroring `ThemeToggle.tsx`'s pattern for `theme`.
